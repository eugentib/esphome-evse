...
class EVSEComponent : public Component {
 public:
  ...
  void set_pilot_adc_pin(int v){pilot_adc_pin_=v;}
 ...
 protected:
  void sample_cp_();
  PilotState classify_state_(uint16_t high_raw) const;
  void update_stable_state_(PilotState sampled);
  void control_();
  void set_pwm_active_(bool active);
  void apply_pilot_output_();
  void open_contactor_();
  void close_contactor_();
  void set_fault_(bool fault, const char *reason=nullptr);
  void publish_();
  float duty_for_current_(float amps) const;
  const char *state_to_string_(PilotState s) const;

  output::FloatOutput *pilot_output_{nullptr};
  switch_::Switch *contactor_{nullptr};
  int pilot_adc_pin_{-1};
  ...
  bool enabled_{false}, pwm_active_{false}, contactor_on_{false}, fault_{false}, diode_ok_{false};
